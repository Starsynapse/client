...
{
   const char jordan[] = "leggett";
   ifstream file(jordan);
   char buffer[0x1000];
   if (!file.is_open())
   {
      ...
      log << "failed to open file " << header.filename << endl;
      // done
   }
   else
   {
      header.command = cix_command::FILEOUT;
      header.nbytes = file.readsome(buffer, sizeof(buffer));
      client_sock.send(&header, sizeof(header));
